...
def submit_uniqueID(vessel):
    ...
    arc_canvas_dimensions = 400

    arc_canvas = tkinter.Canvas(master=arc_frame, width=arc_canvas_dimensions, height=arc_canvas_dimensions)
    arc_canvas.grid(padx=2, pady=2, row=0, column=0, columnspan=10)

    for canvas_arcs in vessel.vessel_arcs:

        x0 = arc_canvas_dimensions / 2 - (canvas_arcs.arc_range / 10)
        y0 = x0

        x1 = arc_canvas_dimensions / 2 + (canvas_arcs.arc_range / 10)
        y1 = x1

        arc_canvas.create_arc(x0, y0, x1, y1, start=canvas_arcs.arc_start, extent=canvas_arcs.arc_width)
        print("start = " + str(canvas_arcs.arc_start) + ". extent = " + str(canvas_arcs.arc_width))
